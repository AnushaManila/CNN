...
for filter_index in range(0, 31):
    ...
    if loss_value > 0:
        ...
        check.append(input_img_data[0])
    end_time = time.time()
    print('Filter %d processed in %ds' % (filter_index, end_time - start_time))
#print ('kept_filters:', len(kept_filters)) #18
np.save('kept_filters5-5.npy', check)
"""
#**************************************************************
# final visualization of the filters
#**************************************************************
savedFilter = np.ravel(np.array(np.load('kept_filters_len40.npy')))
#print ('savedFilter:', savedFilter.shape)
sliced = savedFilter[1:28] # slice array of shape (27,) to be able to reshape next
#print (sliced.shape)
filterVolume = sliced.reshape([3,3,3]) 
#pg.image(filterVolume)

#print (reduce(max, savedFilter.flat))
print (np.amax(sliced))
